...
for interface in /sys/class/net/*/; do
  ...
  if [ ! -d "/sys/class/net/${interface}/device/subsystem" ]; then
    continue;
  fi
  ...
  # /devices/pci0000:00/0000:00:14.0/usb1/1-6/1-6:1.12/net/wwan0
  ...
  #
  # Since we are only interested in the last part of the device address
  ...
  kernel_id=$(udevadm info -q path "/sys/class/net/${interface}" | sed 's /devices/.*/\([^/]\+\)/net/'"${interface}"' \1 g')

  subsystem=$(readlink -f "/sys/class/net/${interface}/device/subsystem")
  subsystem=$(basename "$subsystem")
  ...
  case $subsystem in
    pci)
      ...
      echo "SUBSYSTEM==\"net\", ACTION==\"add\", KERNELS==\"${kernel_id}\", NAME=\"wlan0\"" | \
        sudo tee -a /etc/udev/rules.d/70-persistent-net.rules > /dev/null

      echo "Created rule for: ${interface} on ${kernel_id}"
      ;;
    usb)
      if [ -f /etc/udev/rules.d/70-persistent-net.rules ]; then
        if grep -q wwan0 /etc/udev/rules.d/70-persistent-net.rules; then
          echo "Rule for wwan0 exists. Our work here is done."
          continue
        fi
      fi

      echo "SUBSYSTEM==\"net\", ACTION==\"add\", KERNELS==\"${kernel_id}\", NAME=\"wwan0\"" | \
        sudo tee -a /etc/udev/rules.d/70-persistent-net.rules > /dev/null
